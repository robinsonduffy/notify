...
( function() {
	...
	function generateMarkup() { //build markup
		...
		for ( type in messageTypes ) {
			spans.push( [ '<span class="blackbird-', type, '" type="', type, '"></span>'].join( '' ) );
		}
			...
			'<div class="header">',
				...
				'<div class="right">',
					'<div id="', IDs.controls, '" class="controls">',
						...
						'<span class="blackbird-clear" title="clear" op="clear"></span>',
						'<span class="blackbird-close" title="close" op="close"></span>',
					'</div>',
				'</div>',
			'</div>',
			'<div class="main">',
		...
		if ( span && span.tagName == 'SPAN' ) {
			...
			if ( evt.altKey ) {
				...
				for ( var i = 0; filters[ i ]; i++ ) {
					...
					filters[ i ].className = ( oneActiveFilter || ( spanType == type ) ) ? 'blackbird-' + spanType : 'blackbird-' + spanType + 'Disabled';
					messageTypes[ spanType ] = oneActiveFilter || ( spanType == type );
				}
			}
			else {
				messageTypes[ type ] = ! messageTypes[ type ];
				span.className = ( messageTypes[ type ] ) ? 'blackbird-' + type : 'blackbird-' + type + 'Disabled';
			}
	...
	function resize( size ) {
		...
		span.className = 'blackbird-' + span.title;
